...
const Wait = (props: Props) => {
    ...
    const calculateARTSReceive = () => {
        ...
        return Number(events[wasmIndex]['attributes'][14]?.value || "0") / Math.pow(10, 6)
    }
    
    const calculateUSTReceive = () => {
        const logs = txInfo?.logs;
        if (!logs) return 0;
        const events = logs[1].events;
        return parseInt(events[0]['attributes'][1]?.value || "0") / Math.pow(10, 6)
    }
            ...
            {/* failed */}
            {
                status === STATUS.FAILURE && (
                    <>
                        ...
                        <p className="receipt-text mb-40 font-14">Something went wrong, try again</p>
                        <div className="pr-40 pl-40 buttons-div">
                            <button className="farm-btn mb-28" onClick={_onTryAgain}>
                                <img src={iconRetry} className="mr-4" />
                                <span>Try Again</span>
                            </button>
                            <button className="farm-btn bg-gray mb-28" onClick={_onCancel}>
                                <span>Cancel</span>
                            </button>
                        </div>
                    </>
                        ...
                        <div className="hash-link-div mb-18">
                            ...
                            <div>
                                <div className="center">
                                    ...
                                    <p className="receipt-text font-16">{calculateARTSReceive()} ARTS</p>
                                </div>
                                <div className="center">
                                    <img src={UstIcon} className='mr-4' />
                                    <p className="receipt-text font-16">{calculateUSTReceive()} UST</p>
                                </div>
                            </div>
                        </div>
